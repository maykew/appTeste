...
export class RegisterWorkareaPage implements OnInit, OnDestroy {
  ...
  startUpdatingUserLocation() {
    this.updateInterval = setInterval(async () => {
      ...
      const isInsideWorkArea = this.isMarkerInWorkArea(this.userLocation.latitude, this.userLocation.longitude);

      // Verifique se houve uma transição de estado
      if (isInsideWorkArea !== this.wasInsideWorkArea) {
        // Envie a notificação de entrada ou saída
        this.notificationService.sendNotification(isInsideWorkArea, this.userLocation.latitude, this.userLocation.longitude);
        this.wasInsideWorkArea = isInsideWorkArea;  // Atualize o estado anterior
      }
    }, 5000);
  }
